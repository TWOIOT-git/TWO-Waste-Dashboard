            ...
            <Row>
              {data.map((sensor, i) => (
                <Col md={12} style={colStyle}>
                  <TextWithNumberButtom
                    text={sensor.name}
                    name={sensor[0].sensor_id}
                    number={Math.round(
                      -1 *
                        ((sensor[sensor.length - 1].bin_level / 850) * 100 -
                          100)
                    )}
                  />
                  <GrowthChart
                    data={sensor.map(a => {
                      ...
                      a.time = date.toLocaleTimeString(["en-US"], {
                        weekday: "short",
                        month: "short",
                        day: "numeric",
                        hour: "2-digit"
                      });
                      a.percentage = Math.round(
                        -1 * ((a.bin_level / 900) * 100 - 100)
                      );
                      return a;
                    })}
                  />
